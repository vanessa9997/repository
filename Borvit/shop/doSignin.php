<?php
	require('header.php');

		session_start();
		
		if (empty($_POST['email']))
		{	
			$wszystko_OK=false;
			$_SESSION['e_email']="Podaj poprawny adres e-mail!";
			
			header('Location:signin.php');
			exit();
				
		} else {
		
			//Udana walidacja? Załóżmy, że tak!
			$wszystko_OK=true;
		
			//login
			$login = $_POST['login'];
		
			if ((strlen($login)<3) || (strlen($login)>20))
			{
				$wszystko_OK=false;
				$_SESSION['e_login']="Login musi posiadać od 3 do 20 znaków!";
				
				header('Location:signin.php');
			}
			
			if (ctype_alnum($login)==false)
			{
				$wszystko_OK=false;
				$_SESSION['e_login']="Login może składać się tylko z liter i cyfr (bez polskich znaków)";
				
				header('Location:signin.php');
			}
			
			//hasło
			$password = $_POST['password'];
		
			if ((strlen($password)<8) || (strlen($password)>20))
			{
				$wszystko_OK=false;
				$_SESSION['e_password']="Hasło musi posiadać od 8 do 20 znaków!";
				
				header('Location:signin.php');
			}
		
			$password_hash = password_hash($password, PASSWORD_DEFAULT);
		
			//Imię i nazwisko
			$customer = $_POST['customer'];
		
			if ((strlen($customer)<6) || (strlen($customer)>30))
			{
				$wszystko_OK=false;
				$_SESSION['e_customer']="Podaj poprawne imię i nazwisko";
				
				header('Location:signin.php');
			}
			
			$customer = htmlspecialchars($customer, ENT_QUOTES);
			

			//email
			$email = $_POST['email'];
			$emailB = filter_var($email, FILTER_SANITIZE_EMAIL);
		
			if ((filter_var($emailB, FILTER_VALIDATE_EMAIL)==false) || ($emailB!=$email))
			{
				$wszystko_OK=false;
				$_SESSION['e_email']="Podaj poprawny adres e-mail!";
				
				header('Location:signin.php');
			}
		
			$email = htmlentities($email, ENT_QUOTES, "UTF-8");
		
			//adres
			$address = $_POST['address'];
		
			if ((strlen($address)<15) || (strlen($address)>40))
			{
				$wszystko_OK=false;
				$_SESSION['e_address']="Podaj pełny adres!";
				
				header('Location:signin.php');
			}
			
			$address = htmlspecialchars($address, ENT_QUOTES);

				
			//Zapamiętaj wprowadzone dane do autouzupełniania
			$_SESSION['fr_login'] = $login;
			$_SESSION['fr_password'] = $password;
			$_SESSION['fr_email'] = $email;
			$_SESSION['fr_customer'] = $customer;
			$_SESSION['fr_address'] = $address;
		
			
			
			//Czy email już istnieje?
			$stmt = $conn->prepare("SELECT id FROM users WHERE email='$email'");
			$stmt -> execute();
				
			$ile_takich_maili = $stmt->fetchAll();
					
			if(count($ile_takich_maili)>0)
			{
				$wszystko_OK=false;
				$_SESSION['e_email']="Istnieje już konto przypisane do tego adresu e-mail!";
				
				header('Location:signin.php');
			}		

					
			//wszystko OK
			if ($wszystko_OK==true)
			{
				//zapis do bazy 
				$stmt = $conn->prepare("INSERT INTO users (id, login, password, customer, email, address) VALUES (null, :login, :password, :customer, :email, :address)");
			
				$stmt->bindValue(':login', $login, PDO::PARAM_STR);
				$stmt->bindValue(':password', $password_hash, PDO::PARAM_STR);
				$stmt->bindValue(':customer', $customer, PDO::PARAM_STR);
				$stmt->bindValue(':email', $email, PDO::PARAM_STR);
				$stmt->bindValue(':address', $address, PDO::PARAM_STR);
				$stmt->execute();
				
				$_SESSION['udanarejestracja']=true;
				header('Location: afterSignin.php');
			
			}
				
	
		}
	

	require('footer.php');
?>